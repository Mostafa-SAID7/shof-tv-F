...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, RouterModule, PageShellComponent, SelectControlComponent],
  template: `
    <app-page-shell>
      <main class="min-h-screen bg-background pb-24 pt-32 text-foreground">
        <div class="mx-auto max-w-6xl px-5 lg:px-10">
          <div class="flex flex-col justify-between gap-6 border-b border-white/10 pb-8 sm:flex-row sm:items-end"><div><p class="eyebrow">Your account</p><h1 class="mt-3 text-5xl font-extrabold tracking-tight">Welcome back, {{ store.profile().name.split(' ')[0] }}<span class="text-primary">.</span></h1><p class="mt-3 text-secondary-foreground">{{ store.profile().email }}</p></div><div class="grid h-16 w-16 place-items-center rounded-2xl bg-primary text-xl font-extrabold text-primary-foreground">{{ store.profile().avatarInitials }}</div></div>
           <div class="relative mt-6 h-36 overflow-hidden rounded-3xl border border-white/10"><img src="/pages/account/account.jpg" alt="A quiet place to enjoy your saved films" class="h-full w-full object-cover opacity-70" /><div class="absolute inset-0 bg-gradient-to-r from-background via-background/45 to-transparent"></div><p class="absolute inset-y-0 left-6 flex items-center max-w-xs text-sm font-semibold text-white">Keep your watchlist, progress, and preferences in one place.</p></div>
          <div class="mt-10 grid gap-5 lg:grid-cols-3">
           ...
           <section class="mt-5 rounded-3xl border border-white/10 bg-card p-6"><p class="eyebrow">Preferences</p><div class="mt-5 divide-y divide-white/10"><label for="autoplay" class="flex items-center justify-between gap-4 py-4"><span><span class="block font-semibold">Autoplay next episode</span><span class="mt-1 block text-xs text-muted-foreground">Keep the story moving when an episode ends.</span></span><input id="autoplay" type="checkbox" checked class="h-5 w-5 accent-[#f2b544]" /></label><label for="recommendations" class="flex items-center justify-between gap-4 py-4"><span><span class="block font-semibold">Email recommendations</span><span class="mt-1 block text-xs text-muted-foreground">A small weekly note with things worth watching.</span></span><input id="recommendations" type="checkbox" checked class="h-5 w-5 accent-[#f2b544]" /></label><label for="maturity" class="flex items-center justify-between gap-4 py-4"><span><span class="block font-semibold">Maturity filter</span><span class="mt-1 block text-xs text-muted-foreground">Use your profile controls to keep viewing age appropriate.</span></span><app-select-control id="maturity" [options]="maturityOptions" value="all" ariaLabel="Maturity filter" class="w-44" /></label></div></section>
...
export class AccountComponent {
  ...
  readonly maturityOptions: readonly SelectOption[] = [
    { value: 'all', label: 'All content' },
    { value: '12', label: '12 and under' },
    { value: '7', label: '7 and under' },
  ];
}