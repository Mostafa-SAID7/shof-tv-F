...
For Angular's Vite-backed dev server, pass the runtime `REPLIT_DEV_DOMAIN` through Angular's
`--allowed-hosts` option; the preview hostname can change between sessions.
...
For host errors, update the workflow command through the validated Replit configuration flow instead
of hardcoding a temporary `.replit.dev` hostname or adding an unused `vite.config.js`.